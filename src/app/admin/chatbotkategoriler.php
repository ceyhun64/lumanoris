    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        dataUl.querySelectorAll("li").forEach(li => {
            li.addEventListener("click", async () => {
                ...
                try {
                    const res = await fetch("/lumanoris/admin/ajax/read.php", {
                        method: "POST",
                        body: formData
                    });
        ...
        saveOrUpdateBtn.addEventListener("click", async (event) => {
            ...
            let url = "/lumanoris/admin/ajax/create.php";

            if (id !== "0") {
                url = "/lumanoris/admin/ajax/update.php";
                formData.append("where", "id = " + id);
            }
        ...
        deleteBtn.addEventListener("click", async () => {
            ...
            if (confirm(`Emin misiniz? "${categoryName}" kategorisini kalıcı olarak silmek istiyor musunuz? Bu işlem geri alınamaz.`)) {
                ...
                try {
                    const res = await fetch("/lumanoris/admin/ajax/delete.php", {
                        method: "POST",
                        body: formData
                    });
                    const resText = await res.text();
