...
export default function Chat() {
  ...
  const [chatAdFrequency, setChatAdFrequency] = useState(10); //default number, just in case
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      const conversationId = params.get("convId") || 0;

      const convFetchUrl = conversationId == 0 ? `/api/getconversation.php?chatbot_id=${botId}&user_id=${userId}` : `/api/getconversation.php?chatbot_id=${botId}&user_id=${userId}&convId=${conversationId}`;
      ...
      fetch(convFetchUrl)
        ...
        .then(async data => {
          ...
          // Eğer id:0 ve prompt boş değilse yeni kayıt ekle
          if (data[0]?.id === 0 && initialPrompt.trim() !== "") {
            // İlk 5 kelimeyi al
    ...
    useEffect(() => {
      ...
      const loadHistory = async () => {
        try {
          const historyRes = await fetch(`/api/getchat.php?chatbot_id=${conversation.id}&user_id=1`);
  ...
  const readFileContent = (file) => {
    return new Promise((resolve, reject) => {
      ...
      // PDF için
        ...
        // pdf.js gibi bir kütüphane gerekir
      ...
      // DOCX için
        ...
        reader.onload = async (e) => {
          ...
          const doc = await window.docxParser(arrayBuffer); // örn. `mammoth` kütüphanesi
          resolve(doc.value);
        };
        reader.readAsArrayBuffer(file);
      }
  ...
  // 1. Kullanıcı mesajını hemen arayüze ekle ve DB'ye kaydet
  const userPayload = {
    chatbot_id: conversation.id,
    user_id: 1, // Dinamik kullanıcı ID'si varsa buraya ekle
    sent_by: "user",
    message: data.text,
  };
  ...
  if (data.audioUrl && !data.text && !data.fileName) {
    setMessages((prev) => [...prev, { type: "received", text: "Sesli mesaj alındı..." }]);
    return;
  }

  // Gemini için parçaları hazırla
  let parts = [{ text: data.text }];
  if (data.file) {
    try {
      const base64Data = await fileToBase64(data.file);
      parts.push({ inline_data: { mime_type: data.file.type, data: base64Data } });
    } catch (err) {
  ...
  try {
    ...
    if (envData.status !== "success") throw new Error("API Key alınamadı");
    const apiKey = envData.value;

    // 3. Bot için placeholder oluştur
    const placeholderId = Date.now();
    setMessages((prev) => [
      ...prev,
      { id: placeholderId, type: "received", text: "" }, // Başta boş
    ]);

    // 4. Gemini API'ye istek at (Streaming)
    // NOT: alt=sse parametresi eklemek stream yönetimini kolaylaştırır
    ...
    if (!geminiRes.ok) throw new Error("Gemini API hatası");
    ...
    while (true) {
      ...
      // SSE (Server-Sent Events) formatını parse et
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          try {
            ...
            // Tamamlanmamış JSON chunk'larını es geç
          }
        }
      }
    }

    // 5. Akış bittikten sonra BOT cevabını DB'ye kaydet
    if (fullText) {
      const botPayload = {
        chatbot_id: conversation.id,
        user_id: 1,
  ...
  /*useEffect(() => {
    ...
    if ((prompt || fileName) && messages.length === 0) {
      ...
      // Dosya ve metin varsa ilgili verileri hazırla
      if (fileName) {
        initialMessageData.fileName = fileName;
      }
      if (prompt) {
        initialMessageData.text = prompt;
      }

      // Sadece bir mesaj gönderilecekse (prompt veya fileName varsa)
      if (initialMessageData.text || initialMessageData.fileName) {
        handleSendMessage(initialMessageData);
        setPrompt("");
        setFileName("");
      }
    }
