...
export default function AddToListModal({ isOpen, onClose, lists = [], header = "Listeye Ekle" }) {
    ...
    const handleAddNewList = () => {
        ...
        if (trimmedName && !allLists.includes(trimmedName)) {
    ...
    const handleSave = () => {
        console.log("Kaydedilen Liste:", radioSelection);

        setShowFeedback(true); // feedback göster

        setTimeout(() => {
            setShowFeedback(false); // 2 saniye sonra kapat
            onClose();
            setRadioSelection('');
            setNewListName('');
        }, 2000);
            ...
            <div className="share-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <p className="desc-2" style={{ marginBottom: '20px' }}>
                        Yapay zeka profiline erişimi düzenlemek için mevcut bir listeye ekleyin veya yeni bir liste oluşturun.
                    </p>
                    ...
                    <div className="radio-list">
                        {allLists.map((listName) => (
                            <label key={listName} className="radio-option">
                                <input
                                    type="radio"
                                    checked={radioSelection === listName}
                                    onChange={() => setRadioSelection(listName)}
                                />
