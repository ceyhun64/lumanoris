...
export default function BankInfo({ userId }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch("/api/list_iller.php");
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch(`/api/list_ilceler.php?il=${ilKod}`);
                const data = await res.json();
    ...
    const handleResubmit = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/submerchant_resubmit.php", { method: "POST", body: fd });
            const data = await res.json();
            if (data?.success) {
    ...
    useEffect(() => {
        ...
        const fetchBankInfo = async () => {
            try {
                const res = await fetch(`/api/get_bank_info.php?userId=${userId}`);
                const data = await res.json();
    ...
    const handleSubmit = async () => {
        if (isEditing) {
            ...
            try {
                const res = await fetch("/api/save_bank_info.php", {
                    method: "POST",
                    body: fData
                });
    ...
    return (
        <div className="bank-info-wrapper">
            ...
            <div className="input-with-error mt-10">
                <input type="text" className="input" name="iban" placeholder="TR IBAN NUMARASI" value={formData.iban || ""} onChange={handleChange} disabled={!isEditing} />
            </div>
                    ...
                    <div className="registered-cards">
                        {cards.map((iban, index) => (
                            <div className="card-item" key={index}>
                                ...
                                <span>{iban}</span>
                                <button onClick={() => { setDeleteTarget('iban'); setDeleteModalVisible(true); }} className="delete-btn">Sil</button>
                            </div>
                        ))}
                    </div>
                )}
