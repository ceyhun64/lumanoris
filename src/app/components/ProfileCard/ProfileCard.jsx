...
export default function ProfileCard({bot, comments}) {
        ...
        const fetchUserLists = async (uid) => {
            try {
                ...
                if (Array.isArray(data)) {
                    ...
                    const minimalLists = data.map(list => ({
                        id: list.id,
                        userId: uid,
                        name: list.name
                    }));
    ...
    useEffect(() => {
        ...
        const checkUserFollow = async () => {
            try {
            const res = await fetch("/api/diduserfollow.php", {
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                body: new URLSearchParams({
                data: JSON.stringify({
                    user_id: userId, // aktif kullanıcı ID
                    chatbot_id: bot.id, // profil ID
                }),
                }),
            });
    ...
    if (!userId || !profile.id) {
        alert("Lütfen sepet işlemi için giriş yapın.");
        return;
    }
...
// };
    ...
    const handleNotInterested = async () => {
        ...
        try {
            const response = await fetch('/api/adduninterest.php', {
                method: 'POST',
                body: formData
            });
    ...
    return (
        <div className="profile-card">
            ...
            <div className="profile-top">
                <div className="profile-info">
                    ...
                    <div className='profile-cont'>
                        ...
                        <p className="profile-followers">{profile.follows} takipçi</p>
                        <p className="profile-subtext">{profile.seller}</p>
                    </div>
                </div>
                <div className="profile-actions">
                    <button
                        ...
                        onClick={async () => {
                            try {
                            const res = await fetch("/api/followchatbot.php", {
                                method: "POST",
                                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                                body: new URLSearchParams({
                                data: JSON.stringify({
            ...
            <div className="profile-buttons">
                ...
                </span>{commentCount} Yorum</button>
                <button className='button-ctr' onClick={() => setModalVisible(true)}>
                    <span>
                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path d="M12 8.5625V16.5625M16 12.5625H8" stroke="#FFE6F2" strokeWidth="1.2" strokeLinecap="square" strokeLinejoin="round" />
                        </svg>
                    </span >
                    Listeye Ekle
                </button>
                ...
                <div className="button-buy"
                    ...
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M14.1667 17.082H14.2499M9.16675 17.082H9.24995" stroke="white" strokeWidth="1.44" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M2.5 2.50007H4.2832C4.48424 2.49744 4.67943 2.5677 4.83268 2.69786C4.98592 2.82801 5.08685 3.00926 5.1168 3.20807L5.5168 5.83327M5.5168 5.83327L6.6668 13.3333L15.8332 12.5001L17.5 5.83327H5.5168Z" stroke="white" strokeWidth="0.96" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                </div>
