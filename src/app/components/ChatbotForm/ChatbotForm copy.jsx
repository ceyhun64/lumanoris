...
export default function ChatbotForm({selectedCard, bot}) {
    ...
    if (bot) {
            ...
            const fileMatches = stylePrompt.match(/===== FILE:([^=]+) =====([\s\S]*?)===== END:\1 =====/g) || [];
            const parsedFiles = fileMatches.map(match => {
                const filename = match.match(/===== FILE:([^=]+) =====/)?.[1] || 'unknown';
                const content = match.match(/===== FILE:[^=]+ =====([\\s\\S]*?)===== END:[^=]+ =====/)?.[1]?.trim() || '';
                return { name: filename, content, status: 'done' };
            });
            setUploadedFiles(parsedFiles);
            setFilesBackgroundContent(fileMatches.join('\n'));
            
            // Extract user prompt (after ARKA PLAN section)
            const userPromptMatch = stylePrompt.split('===== ARKA PLAN BELGELERİ SON =====')[1];
            const userPrompt = userPromptMatch ? userPromptMatch.replace('Kullanıcı talimatı:\n', '').trim() : stylePrompt;
...
//console.log(tdata); // Removed for clean extraction
  ...
  const currentPrompt = formData.stylePrompt || "";
  const basePrompt =
    `${content}`;
  ...
  return cleanCurrentPrompt + (cleanCurrentPrompt ? "\n\n" : "") + basePrompt;
};
    ...
    const handleGeneralFileDrop = async (e) => {
        ...
        // --- YÜKLENİYOR DURUMUNU BAŞLAT ---
        setPromptGenerationStatus('processing');
        setFormData(prev => ({...prev, stylePrompt: 'Yükleniyor... Belge içeriği okunuyor, lütfen bekleyiniz...'}));
    ...
    //         setFormData(prev => ({
    //             ...prev,
    //             weeklyPrice: isNaN(numericValue) || numericValue < 0 ? 0 : numericValue
    //         }));
    //     }
    ...
    <div className={`upload-card ${showErrors && errors.profileImage ? 'error' : ''}`}>
        ...
        <div className="card-right">
            ...
            <input type="file" id="profile-upload" accept="image/*" hidden onChange={(e) => handleFileUpload(e, 'profile')} />
                ...
                {/* <div className="image-upload-section">
                    <label className={`upload-btn-sec ${showErrors && errors.coverImage ? 'error' : ''}`} htmlFor="cover-upload">
                    ...
                    <label className={`upload-btn ${showErrors && errors.profileImage ? 'error' : ''}`} htmlFor="profile-upload">
                ...
                {formData.trainingPrompt && (
                    <div className="training-status-area" style={{ marginTop: '10px', padding: '10px', backgroundColor: '#f0f7ff', borderRadius: '8px' }}>
                        <p style={{ fontSize: '13px', color: '#0056b3' }}>
                            <strong>Eğitim Verisi:</strong> {formData.trainingPrompt.length} karakter yüklendi.
                        </p>
                        <button 
                            type="button" 
                            className="clear-training-button"
                            onClick={() => {
                                if(window.confirm("Yüklenen tüm eğitim içeriği silinecek. Emin misin?")) {
                                    setFormData(prev => ({ ...prev, trainingPrompt: '' }));
                                    setUploadedFiles([]); // Dosya listesini de temizle
                                }
                            }}
                            style={{ color: 'red', fontSize: '12px', cursor: 'pointer', border: '1px solid red', padding: '4px 8px', borderRadius: '4px' }}
                        >
                            Eğitimi Temizle
                        </button>
                    </div>
                ...
                <div className="pricing-section-wrapper">
                    ...
                    <div className="pricing-container">
                        <div className="price-setting-area">
                            ...
                            <div className="price-input-group mt-3">
