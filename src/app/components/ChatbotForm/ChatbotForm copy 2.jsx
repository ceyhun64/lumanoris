  ...
  return cleanCurrentPrompt + (cleanCurrentPrompt ? "\n\n" : "") + basePrompt;
    ...
    const handleGeneralFileDrop = async (e) => {
        ...
        reader.onload = async () => {
            ...
            const newPrompt = await generatePromptFromFileContent(file, base64);
            const formattedEntry = `\n--- DOSYA: ${file.name} ---\n${newPrompt}\n`;
            setFileContent(prev => prev => {
                const updated = prev + formattedEntry;
                console.log("Güncel Arka Plan Metni (Toplam):", updated);
                return updated;
            });
            console.log(fileContent);
            // --- YÜKLENİYOR DURUMUNU BİTİR VE METNİ YAZ ---
            setPromptGenerationStatus('completed');
        };
