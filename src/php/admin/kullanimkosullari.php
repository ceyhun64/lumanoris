...
<main class="bg-gray-50 p-6 min-h-screen">
    <div class="max-w-screen-xl mx-auto">
        <?php pageTitle("Kullanım Koşulları", "Bu sayfada sitenizin kullanım koşullarını düzenleyebilirsiniz."); ?>
        <form id="dataForm" class="space-y-6" enctype="multipart/form-data">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token']; ?>">
             <!--form inputs go here-->
            <div class="mt-4">
              <label for="kullanim_kosullari" class="block font-semibold text-sm text-gray-700 mb-2">Kullanım Koşulları</label>
              <textarea id="kullanim_kosullari" name="kullanim_kosullari" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition shadow-sm"></textarea>
            </div>
...
<script>
    ...
    ClassicEditor.create(document.querySelector('#kullanim_kosullari'), editorConfig)
        .then(editor => { 
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        editorInstance.setData(<?= json_encode($kullanim_kosullari) ?>);
        const form = document.getElementById('dataForm');
        form.addEventListener('submit', async function(e) {
            e.preventDefault();
            const editorData = editorInstance.getData();
            const formData = new FormData(this);
            formData.set('kullanim_kosullari', editorData);
            try {
