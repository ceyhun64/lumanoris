        ...
        <>
            <div className="flex flex-col w-full h-full px-16 py-6 text-white relative max-[1100px]:px-4 max-[1100px]:py-8">
                <div className="flex justify-between items-center mb-10 z-10">
                    <h2 className="text-4xl max-[1100px]:text-base font-semibold font-display leading-[56px] max-[1100px]:leading-6 text-transparent bg-clip-text bg-[linear-gradient(323deg,#E879F9_9.6%,#A78BFA_108.66%)]">
                        Diyalog Defteri
                    </h2>

                    <div className="relative">
                        <button
                            className="flex items-center gap-2 px-5 py-3 rounded-xl border border-dashed border-[#E879F9] bg-[#100F17] text-white text-[15px] font-sans cursor-pointer hover:border-[#E879F9] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                            onClick={() => setShowSortMenu((v) => !v)}
                        >
                            <svg width="16" height="10" viewBox="0 0 16 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15.03 0L16 1.07812L8 10L0 1.07812L0.965 0L8 7.83854L15.03 0Z" fill="#E879F9" />
                            </svg>
                        ...
                        {showSortMenu && (
                            <div className="absolute right-0 top-[110%] z-[99] w-[170px] flex flex-col gap-2 px-5 py-3 rounded-lg bg-[#161630] shadow-[-6px_2px_14px_rgba(0,0,0,0.25)]">
                                {sortOptions.map(opt => (
                                    <div
                                        key={opt.value}
                                        className={`flex items-center cursor-pointer text-[13px] font-sans transition-colors duration-150 py-1 ${tab === opt.value ? 'text-fuchsia-400' : 'text-white hover:text-fuchsia-400'}`}
                                        onClick={() => {
                                            setTab(opt.value);
                ...
                {filteredCards.length > 0 && (
                                ...
                                >
                                    {filteredCards.map((card, index) => (
                                        <div
                                            className="relative rounded-lg border border-transparent bg-luma-elevated p-2 mb-2 flex flex-col gap-2 cursor-pointer transition-all duration-200 hover:scale-[1.015] hover:shadow-[0_4px_20px_rgba(217,70,239,0.2)] animate-[fadeInUp_0.4s_ease-out_forwards]"
                                            key={card.id}
                                        ...
                                        >
                                            ...
                                            <div className="flex justify-between items-center px-1">
                                                <div className="flex items-center gap-1.5 min-w-0">
                                                    <img src={card.chatbot_profil_fotografi} alt="" className="w-6 h-6 rounded-full border border-transparent object-cover shrink-0" />
                                                ...
                                                <div className="relative shrink-0" ref={(el) => (menuRefs.current[index] = el)}>
                                                    <button className="flex items-center justify-center w-7 h-7 rounded-lg text-white/40 hover:text-white/80 hover:bg-fuchsia-500/10 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={(e) => { e.stopPropagation(); toggleMenu(index); }} aria-label="Diğer seçenekler">
                                                    ...
                                                    {menuOpenIndex === index && (
                                                        <div className="absolute bottom-8 right-0 z-[99999] min-w-[200px] rounded-xl border border-fuchsia-400/12 bg-[#0E0E22] shadow-[0_8px_32px_rgba(0,0,0,0.55)] overflow-hidden py-1" onClick={(e) => e.stopPropagation()}>
                                                            <ul className="list-none flex flex-col">
                                                                {tab === "mine" ? (<>
                                                                    <li className="flex items-center gap-3 px-3.5 py-2.5 text-[13.5px] text-white/75 hover:bg-fuchsia-500/10 hover:text-white cursor-pointer transition-colors" onClick={(e) => { e.stopPropagation(); setShareOpen(true) }}>
                                                                        <span className="w-5 flex justify-center items-center opacity-70"><svg width="18" height="18" viewBox="0 0 24 25" fill="none"><path opacity="0.3" d="M18 6.5C18.5523 6.5 19 6.05228 19 5.5C19 4.94772 18.5523 4.5 18 4.5C17.4477 4.5 17 4.94772 17 5.5C17 6.05228 17.4477 6.5 18 6.5Z" fill="#E879F9" /><path d="M18 16.58C17.24 16.58 16.56 16.88 16.04 17.35L8.91 13.2C8.96 12.97 9 12.74 9 12.5C9 12.26 8.96 12.03 8.91 11.8L15.96 7.69C16.5 8.19 17.21 8.5 18 8.5C19.66 8.5 21 7.16 21 5.5C21 3.84 19.66 2.5 18 2.5C16.34 2.5 15 3.84 15 5.5C15 5.74 15.04 5.97 15.09 6.2L8.04 10.31C7.5 9.81 6.79 9.5 6 9.5C4.34 9.5 3 10.84 3 12.5C3 14.16 4.34 15.5 6 15.5C6.79 15.5 7.5 15.19 8.04 14.69L15.16 18.85C15.11 19.06 15.08 19.28 15.08 19.5C15.08 21.11 16.39 22.42 18 22.42C19.61 22.42 20.92 21.11 20.92 19.5C20.92 17.89 19.61 16.58 18 16.58Z" fill="#E879F9" /></svg></span>
                                                                        Paylaş
                                                                    </li>
                                                                    ...
                                                                    <>
                                                                    <li className="flex items-center gap-3 px-3.5 py-2.5 text-[13.5px] text-white/75 hover:bg-fuchsia-500/10 hover:text-white cursor-pointer transition-colors" onClick={(e) => { e.stopPropagation(); setSelectedChatbotId(card.conversation_chatbot_id); setShareOpen(true) }}>
                                                                    ...
                                                                    <div className="h-px bg-fuchsia-400/10 mx-2" />
                                                                    <li className="flex items-center gap-3 px-3.5 py-2.5 text-[13.5px] text-white/75 hover:bg-fuchsia-500/10 hover:text-white cursor-pointer transition-colors" onClick={(e) => handleHideBot(e, card.conversation_chatbot_id)}>
