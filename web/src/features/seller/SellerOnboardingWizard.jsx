...
function parseDiffgramRows(xmlString) {
  ...
  try {
    ...
    const rowNodes = dataSet
      ? dataSet.children
      : doc.getElementsByTagName("Temp");
    const out = [];
    for (let i = 0; i < rowNodes.length; i++) {
      ...
      for (let j = 0; j < row.childNodes.length; j++) {
        ...
        if (child.nodeType === 1)
          entry[child.localName || child.nodeName] = child.textContent;
      }
      out.push(entry);
    }
    return out;
...
function normalizeIller(items) {
  ...
  if (items.length === 1 && items[0] && typeof items[0].any === "string") {
    return parseDiffgramRows(items[0].any)
      .map((r) => ({
...
function normalizeIlceler(items) {
  ...
  if (items.length === 1 && items[0] && typeof items[0].any === "string") {
    return parseDiffgramRows(items[0].any)
      .map((r) => ({
        Ilce_Kodu:
          parseInt(r.ID ?? r.TBL_MERNIS_ILCEKOD ?? r.ILCE_KOD ?? r.Kodu, 10) ||
          0,
...
/**
 * İl/ilçe listesi Param pazaryeri servisinden geliyor. Servis bu ortamda
 * bağlı değilse ("dev stub"), kullanıcının kendi Param hesabında yapabileceği
 * hiçbir şey yok — eski metin onu boşuna oraya yönlendiriyordu.
 */
function describeRefDataError(message) {
  const text = String(message || "");
  if (text.includes("dev stub")) {
    return "Ödeme sağlayıcısı (Param POS) entegrasyonu bu ortamda devrede değil, bu yüzden il/ilçe listesi alınamıyor. Satıcı kaydı entegrasyon tamamlanana kadar tamamlanamaz.";
  }
  return text
    ? "İl/ilçe listesi alınamadı: " + text
    : "İl/ilçe listesi alınamadı. Lütfen daha sonra tekrar deneyin.";
}
...
export default function SellerOnboardingWizard({
  ...
  // Yalnızca gerçekten gönderilmiş bir başvurunun reddi.
  const [paramError, setParamError] = useState(initialStatus?.lastError || "");
  // İl/ilçe listesi gibi referans verisi çekilemediğinde. Başvuru reddi
  // DEĞİL — ikisi aynı state'i paylaşınca hiç başvurmamış kullanıcıya
  // "Önceki başvuru reddedildi" gösteriliyordu.
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/wallet/get_bank_info.php?userId=${userId}`,
        );
    ...
    (async () => {
      try {
        ...
        if (!cancelled && data?.success) {
          ...
          setRefDataError(describeRefDataError(data?.message));
        }
      } catch (err) {
        if (!cancelled) setRefDataError(describeRefDataError(err.message));
      } finally {
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      if (isCorporate) {
        ...
        ["full_name", "id_number", "tax_office", "kisi_dogum_tarihi"].forEach(
          (k) => {
            if (!form[k]?.toString().trim()) e[k] = true;
          },
        );
  ...
  const submit = async () => {
    ...
    try {
      ...
      const saveRes = await fetch("/api/wallet/save_bank_info.php", {
        method: "POST",
        body: fd,
      });
      ...
      const regRes = await fetch("/api/seller/submerchant_register.php", {
        method: "POST",
        body: regFd,
      });
  ...
  const selectedIl = useMemo(
    () =>
      iller.find(
        (i) =>
          String(i.IL_Kodu ?? i.Il_Kodu ?? i.IL_KOD ?? i.kod) ===
          String(form.il_kod),
      ),
    [iller, form.il_kod],
  );
    ...
    <div className="flex flex-col gap-6 rounded-2xl border border-transparent bg-luma-elevated p-6">
      <div>
        <h2 className="font-display text-xl font-semibold text-white">
          Pazaryeri Satıcı Kaydı
        </h2>
        <p className="mt-1 text-sm text-white/55">
          Chatbot satabilmek için Param pazaryerine satıcı olarak kaydolun.
        </p>
      </div>
      ...
      {refDataError && (
        <div className="rounded-xl border border-amber-400/40 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
          <strong>Adres bilgileri yüklenemedi:</strong> {refDataError}
        </div>
      )}
            ...
            {[
              {
                ...
                title: "Bireysel",
                desc: "TC Kimlik no ile şahıs olarak satış yapacağım.",
              },
              {
                value: "Şahıs Şirketi",
                title: "Şahıs Şirketi",
                desc: "TC Kimlik no + vergi dairesi ile şahıs şirketi olarak satış yapacağım.",
              },
              {
                value: "Kurumsal Hesap",
                title: "Kurumsal",
                desc: "Vergi numaralı şirket (Limited/Anonim) adına satış yapacağım.",
              },
            ].map((opt) => (
              <button
                key={opt.value}
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {isCorporate ? (
            <>
              <Input
                ...
                value={form.authorized_first_name}
                onChange={(e) =>
                  update("authorized_first_name", e.target.value)
                }
              />
              <Input
                placeholder="Yetkili Soyadı"
                value={form.authorized_last_name}
                onChange={(e) => update("authorized_last_name", e.target.value)}
              />
              <Input
                placeholder="Şirket Ünvanı"
                value={form.company_title}
                onChange={(e) => update("company_title", e.target.value)}
              ...
              />
              <Input
                placeholder="Yetkilinin TC Kimlik No (11 hane)"
              ...
              <label className="flex flex-col gap-1.5 text-xs text-white/55">
                ...
                <BirthdatePicker
                  value={ddmmyyyyToIso(form.yetkili_kisi_dogum_tarihi)}
                  onChange={(iso) =>
                    update("yetkili_kisi_dogum_tarihi", isoToDdmmyyyy(iso))
                  }
                />
              </label>
            </>
            ...
            <>
              ...
              />
              <Input
                placeholder="Ticari Ünvan (opsiyonel)"
                value={form.company_title}
                onChange={(e) => update("company_title", e.target.value)}
                className="sm:col-span-2"
              />
              ...
              <label className="flex flex-col gap-1.5 text-xs text-white/55 sm:col-span-2">
                Doğum Tarihi
                <BirthdatePicker
                  value={ddmmyyyyToIso(form.kisi_dogum_tarihi)}
                  onChange={(iso) => update("kisi_dogum_tarihi", isoToDdmmyyyy(iso))}
                />
              </label>
            </>
          ) : (
          ...
          />
          <Input
            placeholder="TR ile başlayan IBAN (26 karakter)"
            value={form.iban}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          >
            <SelectTrigger>
              <SelectValue
                placeholder={
                  loadingIller ? "İl listesi yükleniyor..." : "İl seçin"
                }
              />
            </SelectTrigger>
            <SelectContent>
              {iller.map((il) => {
                ...
                return (
                  <SelectItem key={kod} value={String(kod)}>
                    {ad}
                  </SelectItem>
                );
              })}
          ...
          >
            <SelectTrigger>
              <SelectValue
                placeholder={
                  loadingIlceler ? "İlçeler yükleniyor..." : "İlçe seçin"
                }
              />
            </SelectTrigger>
            <SelectContent>
          ...
          <Input
            placeholder="Cadde"
            value={form.cadde}
            onChange={(e) => update("cadde", e.target.value)}
          />
          <Input
            placeholder="Sokak"
            value={form.sokak}
            onChange={(e) => update("sokak", e.target.value)}
          />
          <div className="grid grid-cols-3 gap-3 sm:col-span-2">
            <Input
              placeholder="Bina No"
              value={form.bina_no}
              onChange={(e) => update("bina_no", e.target.value)}
            />
            <Input
              placeholder="Kapı No"
              value={form.kapi_no}
              onChange={(e) => update("kapi_no", e.target.value)}
            />
            <Input
              placeholder="Posta Kodu"
              value={form.posta_kodu}
              onChange={(e) => update("posta_kodu", e.target.value)}
            />
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="flex flex-col gap-4">
          ...
          <dl className="grid grid-cols-[auto,1fr] gap-x-4 gap-y-2 rounded-xl bg-luma-input p-4 text-sm [&_dt]:text-white/50 [&_dd]:text-white">
            <dt>Hesap Tipi</dt>
            <dd>{form.account_type}</dd>
            {isCorporate ? (
              <>
                <dt>Yetkili</dt>
                <dd>
                  {form.authorized_first_name} {form.authorized_last_name}
                </dd>
                <dt>Şirket</dt>
                <dd>{form.company_title}</dd>
                <dt>Vergi No / Daire</dt>
                <dd>
                  {form.tax_number} / {form.tax_office}
                </dd>
                <dt>Yetkili TC</dt>
                <dd>{form.id_number}</dd>
                <dt>Yetkili Doğum Tarihi</dt>
                <dd>{form.yetkili_kisi_dogum_tarihi}</dd>
              </>
            ) : isSahis ? (
              <>
                <dt>Ad Soyad</dt>
                ...
                <dd>{form.id_number}</dd>
                <dt>Vergi Dairesi</dt>
                <dd>{form.tax_office}</dd>
                <dt>Doğum Tarihi</dt>
                <dd>{form.kisi_dogum_tarihi}</dd>
              </>
            ) : (
              <>
                <dt>Ad Soyad</dt>
            ...
            <dd>
              {selectedIl
                ? (selectedIl.IL_Adi ?? selectedIl.Il_Adi ?? form.il_kod)
                : form.il_kod}{" "}
              / {form.ilce_kod}
            </dd>
            <dt>Adres</dt>
            <dd>
              {[
                ...
                form.cadde,
                form.sokak,
                form.bina_no && `No: ${form.bina_no}`,
                form.kapi_no && `Daire: ${form.kapi_no}`,
                form.posta_kodu,
              ]
                .filter(Boolean)
                .join(" ")}
          ...
          <p className="text-xs text-white/50">
            Onayladığınızda Param tarafına gönderilir ve aktivasyon başlatılır.
          </p>
        </div>
      )}
